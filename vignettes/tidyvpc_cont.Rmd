...
* By providing traditional binning methods and new [binless methods](https://doi.org/10.1002/psp4.12319)
...
```{r message=FALSE, fig.width = 9, fig.height = 6, fig.align='center', out.width="90%"}
vpc <- observed(obs_data, x=TIME, y=DV) %>%
    simulated(sim_data, y=DV) %>%
    binning(bin = NTIME)
```
...
```{r fig.width = 9, fig.height = 6, fig.align='center', out.width="90%"}
vpc <- observed(obs_data, x=TIME, y=DV) %>%
    simulated(sim_data, y=DV) %>%
    binning(bin = NTIME) %>%
    vpcstats()
...
```{r warning = FALSE, fig.width = 9, fig.height = 6, fig.align='center', out.width="90%"}
...
```{r  fig.width = 9, fig.height = 6, fig.align='center', out.width="90%", warning = FALSE}
vpc <- observed(obs_data, x=TIME, y=DV) %>%
    simulated(sim_data, y=DV) %>%
...
```{r  fig.width = 9, fig.height = 6, fig.align='center', out.width="90%", warning = FALSE, eval = FALSE}
...
You can customize the colors of the censoring plots using the `censoring.color` and `censoring.fill` arguments. The `censoring.color` argument accepts a named vector with `"observed"` and `"simulated"` elements for line colors, while `censoring.fill` controls the ribbon fill color.
...
For more advanced customization using ggplot2 functions, use `censoring.output = "list"` to return individual ggplot objects that can be modified directly:
